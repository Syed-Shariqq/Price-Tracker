      ...
      <form
        ...
        <div className='w-full font-bold text-xl flex items-center justify-evenly gap-3 p-5'>
          <h1
            ...
            className={`tab-item cursor-pointer text-lg px-4 py-2 ${activeTab === 'signup' ? 'active' : 'inactive'}`}>Sign Up</h1>
          <h1
            onClick={() => setActiveTab('login')}
            className={`tab-item cursor-pointer text-lg px-4 py-2 ${activeTab === 'login' ? 'active' : 'inactive'}`}>Log In</h1>
        </div>
        <div className='flex flex-col items-center justify-center'>
          <h1 className='text-lg font-extrabold tracking-tight text-gray-900'>Log In Form</h1>
        ...
        <div className='flex flex-col items-center justify-center gap-3'>
          <div className='flex items-center justify-center'>
            ...
            <input
              ...
              className='w-64 transition-all focus:border-b-2 focus:border-blue-500 2xl:w-100 2xl:text-lg 2xl:h-14 md:h-12 md:text-xl md:w-80 outline-none border-b-2 border-gray-600 h-10 p-2'
              name='email'
          ...
          <button
            disabled={loading}
            className={`w-64 ${loading ? 'cursor-not-allowed opacity-50' : ''} hover:bg-blue-700 hover:scale-105 transition-all duration-300 active:scale-95 2xl:w-100 md:h-12 md:text-md cursor-pointer md:w-80 outline-none mt-5 h-10 rounded-full bg-blue-500 text-white font-semibold`}>{loading ? 'Logging In' : 'Log Into My Account'}
          </button>
        </div>
      </form>

      <Link to="/forgot-password"
        className='text-sm md:text-md cursor-pointer hover:text-blue-900 transition-all duration-300 mt-4 text-blue-600'>
        <p>Forgot Password?</p>
      </Link>

      {/* Don't have an account section */}
      <div className='flex my-5 items-center justify-center'>
        <p className='text-sm md:text-md text-gray-500'>Don't have an account?
          <span
            onClick={() => setActiveTab('signup')}
